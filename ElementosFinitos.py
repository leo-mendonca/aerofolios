...
def coeficientes_auxiliares_ordem2(a, b, c, d, e, f, x, y):
    ...
    ax = 2 * d * (x[1] - x[0]) + f * (y[1] - y[0])
    bx = 2 * d * (x[2] - x[0]) + f * (y[2] - y[0])
    cx = b + 2 * d * x[0] + f * y[0]
    ay = 2 * e * (y[1] - y[0]) + f * (x[1] - x[0])
    by = 2 * e * (y[2] - y[0]) + f * (x[2] - x[0])
    cy = c + 2 * e * y[0] + f * x[0]
    return ax, bx, cx, ay, by, cy


def teste_laplace(nome_malha=None, tag_fis=None, ordem=1, gauss=False, n_teste=1):
    ...
    A, b = fea.matriz_laplaciano_escalar(contornos_dirichlet, ordem=ordem, verbose=True, gauss=gauss)
...
class FEA(object):
    ...
    def matriz_laplaciano_escalar(self, contornos_dirichlet=[], contornos_neumann=[], contornos_livres=[], ordem=1, verbose=False, gauss=False):
        ...
        for i in pontos_sem_dirichlet:  # aqui, varremos todos os pontos nos quais a funcao teste nao eh identicamente nula
            ...
            for l in elementos_i:  # varremos cada elemento no qual N_i != 0
                ...
                if ordem == 2:
                    ...
                    axi, bxi, cxi, ayi, byi, cyi = coeficientes_auxiliares_ordem2(ai, bi, ci, di, ei, fi, x, y)
                ...
                for j in elementos[l]:  # varremos os pontos do elemento l
                    ...
                    if gauss:
                        ...
                        axj, bxj, cxj, ayj, byj, cyj = coeficientes_auxiliares_ordem2(aj, bj, cj, dj, ej, fj, x, y)
...
if __name__ == "__main__":
    ...
    # nome_malha, tag_fis= Malha.malha_quadrada("teste", 0.03)
    nome_malha = "Malha/teste.msh"
    teste_laplace(nome_malha, tag_fis, ordem=2, gauss=False, n_teste=2)
    teste_laplace(nome_malha, tag_fis, ordem=1, gauss=False, n_teste=2)
    print("r")
